...
1. `.gitignore`에 `.env`, `*.db` 추가
2. `better-sqlite3`, `drizzle-orm`, `dotenv`, `pino`, `antd`, `sass`, `lightweight-charts`, `zustand` 설치
3. `main/lib/toss-api/` 아래 OAuth 클라이언트 + 레이트리미터 스켈레톤 작성 (조회 API만 우선 구현)
4. `main/db/schema.ts` + 초기 마이그레이션 작성 (1차 범위 테이블만)
5. 전략 평가 엔진 + Electron Notification 연동 스켈레톤 작성
6. 토스 WTS에서 client_id/secret 발급 및 허용 IP 등록 (사용자 액션)
