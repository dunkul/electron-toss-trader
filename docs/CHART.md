...
| 실시간 갱신         | WS 틱으로 오늘 봉의 고가/저가/종가/거래량을 갱신(거래량은 각 체결 수량을 계속 더해나감)  |
...
| 이동평균선(P0-1)    | 5/20/60/120봉 SMA 오버레이, 켜기/끄기 토글                                            |
| OHLC 상단 레전드(P0-2) | 호버 시 해당 캔들, 평소엔 오늘봉(실시간 갱신) OHLC+거래량 표시                     |
| 봉 단위 전환(P0-3)  | 일봉 ↔ 1분봉 `Segmented` 토글                                                        |
| 거래량 이동평균선(P1-4) | 거래량 서브패널에 20기간 SMA, 토글 없이 항상 표시                                  |
...
- 1분봉의 조회 가능 범위는 **당일 장중만** 된다(실기기 확인). API가 `nextBefore`를 계속 내려주면서
  실제 `before` 페이지 요청은 에러를 던지기 때문에, 예상과 달리 무한 스크롤이 "더 이상 이전 데이터
  없음"으로 조용히 멈추지 않고 스크롤할 때마다 실패 토스트가 연달아 뜬다. 그래서 `handleLoadMore`
  맨 앞에 `candleInterval === '1m'`이면 즉시 return하는 가드를 추가해 애초에 `before` 호출 자체를
  하지 않도록 막았다(1분봉에서는 무한 스크롤 이전 페이지 로딩 기능을 아예 비활성화).
- 장 시간 외 구간의 갭 — 캔들이 실제 데이터 포인트 단위(ordinal x축)라 문제없음을 확인.

---

### P1-4. 거래량 이동평균선(20) (완료)
...
- 오늘봉 거래량이 실시간 틱으로 갱신되므로(2장 참고), 이 MA도 P0-1의 가격 MA와 같은 방식으로
  틱마다 오늘봉 값만 다시 계산해서 `update()`하면 같이 살아있게 만들 수 있다.
